...
@section('sub-sidebar')
    @include('pages.major.partials.sidebar')
@endsection
    ...
    <div class="content">
        ...
        <div class="block">
            <div class="block-header block-header-default">
                <h3 class="block-title">All Major</h3>
                ...
                <div class="block-options">
                    <a href="{{ route('addMajor') }}" class="btn btn-outline-primary">
                        Add Major
                    </a>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        @else
                            @foreach($majors as $major)
                                <tr>
                                    <td class="text-center">{{ (($majors->currentPage() - 1 ) * $majors->perPage() ) + $loop->iteration }}</td>
                                    <td class="font-w600">{{ $major->major_name }}</td>
                                    <td class="font-w600">{{ $major->fieldOfStudy->field_of_study_name }}</td>
                                    <td class="text-center">
                                        <div class="btn-group">
                                            {{--  <button type="button" class="btn btn-sm btn-alt-secondary" data-toggle="tooltip" title="Edit">  --}}
                                                <a href="{{ route('editMajor', $major->major_id) }}" class="btn btn-sm btn-alt-primary pull-right"><i class="si si-pencil"></i></a>
                                            {{--  </button>  --}}
