    ...
    <div class="content">
        ...
        <div class="block">
            <div class="block-header block-header-default">
                <h3 class="block-title">All User Group</h3>
                ...
                <div class="block-options">
                    <a href="{{ route('addUserGroup') }}" class="btn btn-outline-primary">
                        Add Group
                    </a>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        @else
                            @foreach($userGroups as $userGroup)
                                <tr>
                                    <td class="text-center">{{ (($userGroups->currentPage() - 1 ) * $userGroups->perPage() ) + $loop->iteration }}</td>
                                    <td class="font-w600">{{ $userGroup->user_group_name }}</td>
                                    <td class="font-w600">
                                        @foreach($userGroup->insitutions as $i => $detail)
                                            {{ $detail->user->name }},
                                        @endforeach
                                    </td>
                                    <td class="text-center">
                                        {{-- <div class="btn-group"> --}}
                                            <a href="{{ route('editUserGroup', $userGroup->user_group_id) }}" class="btn btn-sm btn-alt-secondary" data-toggle="tooltip" title="Edit"><i class="si si-pencil"></i></a>
                                            <a href="{{ route('deleteUserGroup', $userGroup->user_group_id) }}" class="btn btn-sm btn-alt-secondary" data-toggle="tooltip" title="Delete"><i class="si si-trash"></i></a>
                                        {{-- </div> --}}
                                    </td>
                                </tr>
                            @endforeach
