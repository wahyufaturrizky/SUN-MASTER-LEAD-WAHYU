    ...
    <div class="content">
        ...
        <div class="block">
            <div class="block-header block-header-default">
                <h3 class="block-title">All Institution Contact</h3>
                ...
                <div class="block-options">
                    <a href="{{ route('addInstitutionContact') }}" class="btn btn-outline-primary">
                        Add Contact
                    </a>
                </div>
            </div>
            ...
            <div class="block-content">
                ...
                <table class="table table-striped table-vcenter">
                    ...
                    <tbody>
                        @if($institutionContacts->isEmpty())
                            <tr>
                                <td colspan="9" class="text-center">Data Not Found</td>
                            </tr>
                        @else
                            @foreach($institutionContacts as $institutionContact)
                                <tr>
                                    <td class="text-center">{{ (($institutionContacts->currentPage() - 1 ) * $institutionContacts->perPage() ) + $loop->iteration }}</td>
                                    <td class="font-w600">{{ $institutionContact->name }}</td>
                                    <td>{{ $institutionContact->position }}</td>
                                    <td>{{ $institutionContact->address }}</td>
                                    <td>{{ $institutionContact->phone }}</td>
                                    <td>{{ $institutionContact->email }}</td>
                                    <td class="font-w600">{{ $institutionContact->type }}</td>
                                    <td class="font-w600">{{ $institutionContact->reference() }}</td>
                                    <td class="text-center">
                                        {{-- <div class="btn-group"> --}}
                                            <a href="{{ route('editInstitutionContact', $institutionContact->institution_contact_id) }}" class="btn btn-sm btn-alt-secondary" data-toggle="tooltip" title="Edit"><i class="si si-pencil"></i></a>
                                            <a href="{{ route('deleteInstitutionContact', $institutionContact->institution_contact_id) }}" class="btn btn-sm btn-alt-secondary" data-toggle="tooltip" title="Delete"><i class="si si-trash"></i></a>
                                        {{-- </div> --}}
                                    </td>
                                </tr>
                            @endforeach
