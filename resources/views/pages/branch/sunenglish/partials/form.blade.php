...
<div class="form-group row {{ $errors->has('branch_sun_education_id') ? ' is-invalid' : '' }}">
    <div class="col-12">
        <div class="form-material">
            <select class="js-select2 form-control" id="branch_sun_education_id" name="branch_sun_education_id" data-placeholder="Select one">
                <option value="">Select one</option><!-- Required for data-placeholder attribute to work with Select2 plugin -->
                {{-- @foreach($sunEducationBranches as $sunEducationBranch)
                    <option value="{{ $sunEducationBranch->branch_sun_education_id }}">{{ $sunEducationBranch->branch_name }}</option>
                @endforeach --}}
                @foreach($sunEducationBranches as $sunEducationBranch)
                    @isset($branch)
                        @if($sunEducationBranch->branch_sun_education_id == $branch->branch_sun_education_id)
                            <option value="{{ $sunEducationBranch->branch_sun_education_id }}" selected>{{ $sunEducationBranch->branch_name }}</option>
                        @else
                            <option value="{{ $sunEducationBranch->branch_sun_education_id }}">{{ $sunEducationBranch->branch_name }}</option>
                        @endif
                    @else
            ...
            <label for="branch_sun_education_id">Sun Education Branch (for auto branching)</label>
            @if ($errors->has('branch_sun_education_id'))
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('branch_sun_education_id') }}</strong>
                </span>
            @endif
        </div>
    </div>
